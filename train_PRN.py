...
def main():
    ...
    dataset_train = Dataset(data_path=opt.data_path,prefix=opt.prefix)
...
#       model.load_state_dict(torch.load(os.path.join(opt.save_path, 'net_epoch%d.pth' % initial_epoch)))
    ...
    def compute_M(x, y, batch_num):
        ...
        with torch.no_grad():
            for i, w in enumerate(model.parameters()):
                ...
                Importance[i].add_(torch.abs(w.grad.data))
        model.zero_grad()
        optimizer.zero_grad()

    if opt.resume:
        print("Resume Training...")
        model.load_state_dict(torch.load(os.path.join(opt.save_path, 'net_latest.pth')))
        Importance = torch.load(os.path.join(opt.save_path,'Importance.pth'))
        Star_vals = torch.load(os.path.join(opt.save_path, 'Star.pth'))
    else:
        print("first task...")
        Importance = []
        Star_vals = []
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (input_train, target_train) in enumerate(loader_train, 0):
            ...
            if opt.resume:
                for i, w in enumerate(model.parameters()):
                    loss += opt.beta / 2 * torch.sum(torch.mul(Importance[i], torch.abs(w - Star_vals[i]))) + \
                            opt.beta / 4 * torch.square(
                        torch.sum(torch.mul(Importance[i], torch.abs(w - Star_vals[i]))))
        ...
        if epoch % opt.save_freq == 0:
            torch.save(model.state_dict(), os.path.join(opt.save_path, opt.name+'_'+'net_epoch_%d.pth' % (epoch+1)))
        if epoch == opt.epochs - 1:
            with torch.no_grad():
                for i, w in enumerate(model.parameters()):
                    Star_vals[i].copy_(w)
            torch.save(Importance, os.path.join(opt.save_path, 'Importance.pth'))
            torch.save(Star_vals, os.path.join(opt.save_path, 'Star.pth'))
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.data_path.find('RainTrainH') != -1:
            ...
            prepare_data_RainTrainH(data_path=opt.data_path, patch_size=100, stride=80, prefix=opt.prefix)
        elif opt.data_path.find('RainTrainL') != -1:
            prepare_data_RainTrainL(data_path=opt.data_path, patch_size=100, stride=80, prefix=opt.prefix)
