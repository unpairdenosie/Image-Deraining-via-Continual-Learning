...
def main():
    ...
    os.makedirs(os.path.join(opt.save_path, opt.prefix+'test_x'), exist_ok=True)
    ...
    model.load_state_dict(torch.load(os.path.join(opt.test_model)))
    model.eval()
    ...
    for img_name in os.listdir(opt.data_path):
        if is_image(img_name):
            ...
            cv2.imwrite(os.path.join(opt.save_path, opt.prefix+'test_x', 'no'+img_name), save_out)
            count += 1
