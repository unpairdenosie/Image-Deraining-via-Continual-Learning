...
#       model.load_state_dict(torch.load(os.path.join(opt.save_path, 'net_epoch%d.pth' % initial_epoch)))
    ...
    for w in model.parameters():
        Importance.append(torch.rand_like(w)/0.1)
    if opt.task_id > 0:
...
#            if step % 40 == 0 and opt.resume:
#                writer.add_scalar('Rain100L/Train_loss', tt_loss/40, step)
#                tt_loss = 0.0

            if opt.task_id > 0:
                for i, w in enumerate(model.parameters()):
                    loss += (opt.beta / 2 * torch.mean(torch.mul(All_Importance[i], torch.abs(w - Star_vals[i])))
                    + opt.beta / 2 * torch.square(torch.mean(torch.mul(All_Importance[i], torch.abs(w - Star_vals[i])))))
